...
pub fn new_map_rooms_and_corridors() -> (Rect, Vec<TileType>) {
...
fn apply_room_to_map(room: &Rect, map: &mut [TileType]) {
    for y in room.y1 + 1..=room.y2 {
        for x in room.x1 + 1..=room.x2 {
            map[get_index_at(x, y)] = TileType::Floor;
        }
    }
}
...
/// Makes a map with wall boundaries and randomly placed walls
pub fn new_map_deprecated() -> Vec<TileType> {
